  ...
  <div class="analyse center">
    ...
    <div v-for="(li, i) in events" :key="i" class="box event-wrap">
      <p>
        ...
        {{ li.team == 1 ? li.player : '' }}
        <img :src="li.team == 1 && li.name == '替补' ? li.in : ''" alt />
        {{ li.team == 1 && li.player2 ? li.player2 : '' }}
        <img :src="li.team == 1 && li.name == '替补' ? li.out : ''" alt />
      </p>
      ...
      <p>
        ...
        {{ li.team == 2 ? li.player : '' }}
        <img :src="li.team == 2 && li.name == '替补' ? li.in : ''" alt />
        {{ li.team == 2 && li.player2 ? li.player2 : '' }}
        <img :src="li.team == 2 && li.name == '替补' ? li.out : ''" alt />
      </p>
    </div>
    ...
    <div>
      ...
      <div v-for="(li, i) in statics" :key="i" class="box event-wrap">
        <p>
          {{ li.type_en_name == 'BallPossession' ? parseInt(li.team1) + '%' : parseInt(li.team1) }}
        </p>
        <p>{{ li.type_cn_name }}</p>
        <p>
          {{ li.type_en_name == 'BallPossession' ? parseInt(li.team2) + '%' : parseInt(li.team2) }}
        </p>
      </div>
    </div>
    <h2>联赛积分榜</h2>
    ...
    <div class="history">
      <div class="title">
        <span v-for="(li, i) in history_tab" :key="i" @click="history_check(i)" :class="{ active: history_index === i }">{{ li }}</span>
      ...
      <div class="box event-wrap">
        <el-table :data="history_data" style="width: 100%">
          ...
          <el-table-column label="半场" width="50">
            <template slot-scope="scope">{{ scope.row.home_team_half_time_score }}-{{ scope.row.away_team_half_time_score }}</template>
          </el-table-column>
          ...
          <el-table-column prop="sheng" label="胜负" width="70">
            <template slot-scope="scope">
              <span :class="{ red: scope.row.sheng == '胜', green: scope.row.sheng == '负', blue: scope.row.sheng == '平' }">{{ scope.row.sheng }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="ya" label="亚盘" width="70">
            <template slot-scope="scope">
              <span :class="{ red: scope.row.ya == '赢', green: scope.row.ya == '输', blue: scope.row.ya == '走' }">{{ scope.row.ya }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="qiu" label="大小球" width="70">
            <template slot-scope="scope">
              <span :class="{ red: scope.row.qiu == '大', green: scope.row.qiu == '小', blue: scope.row.qiu == '走' }">{{ scope.row.qiu }}</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div class="txt">
        近{{ parseInt(this.history_page) }}场：胜出 <span class="red">{{ historycount.sheng.length }}</span> 场， 平局
        <span class="blue">{{ historycount.ping.length }}</span> 场， 负 <span class="green">{{ historycount.fu.length }}</span> 场， 胜率:
        <span class="red">{{ (historycount.sheng.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 赢盘率:
        <span class="red">{{ (historycount.ying.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 大球率:
        <span class="red">{{ (historycount.da.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
      </div>
    </div>
    ...
    <div class="latest history">
      ...
      <div class="txt">
        近{{ parseInt(this.history_page) }}场：胜出 <span class="red">{{ homecount.sheng.length }}</span> 场， 平局
        <span class="blue">{{ homecount.ping.length }}</span> 场， 负 <span class="green">{{ homecount.fu.length }}</span> 场， 胜率:
        <span class="red">{{ (homecount.sheng.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 赢盘率:
        <span class="red">{{ (homecount.ying.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 大球率:
        <span class="red">{{ (homecount.da.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
      </div>
    </div>
    <div class="latest history">
      <div class="title">
      ...
      <div class="txt">
        近{{ parseInt(this.history_page) }}场：胜出 <span class="red">{{ awaycount.sheng.length }}</span> 场， 平局
        <span class="blue">{{ awaycount.ping.length }}</span> 场， 负 <span class="green">{{ awaycount.fu.length }}</span> 场， 胜率:
        <span class="red">{{ (awaycount.sheng.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 赢盘率:
        <span class="red">{{ (awaycount.ying.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
        , 大球率:
        <span class="red">{{ (awaycount.da.length / parseInt(history_page).toFixed(2)) * 100 }}%</span>
      </div>
    </div>
  </div>
</template>
      ...
      data.map(i => {
        ...
        if (i.match_daxiaoqiu) {
          ...
          if (daxiaoqiu) {
            ...
            i['qiupan'] = (i.home_team_score + i.away_team_score - daxiaoqiu.ovalue).toFixed(2)
          }
        }
